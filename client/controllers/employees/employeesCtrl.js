angular.module("employees").controller("employeesCtrl", ['$scope', '$rootScope', 'employeeSvc', function ($scope, $rootScope, employeeSvc) {
    ...
    //        })
    //    }
    employeeSvc.getEmployeesFromApi().then(function (response) {
        $scope.employees = response;
    }).catch(function (response) {
        $scope.error = response;
    });
}]).controller("employeeDetailsCtrl", ['$scope', '$rootScope', 'employeeSvc', function ($scope, $rootScope, employeeSvc) {
    employeeSvc.getEmployeeFromApi().then(function (response) {
...
}]).controller("editEmployeeCtrl", ['$scope', '$rootScope', 'employeeSvc', function ($scope, $rootScope, employeeSvc) {
    employeeSvc.getEmployeeFromApi().then(function (response) {
        $scope.employee = response;
    }).catch(function (response) {
        $scope.error = response;
    ...
    $scope.editEmployee = function (employee) {
        employeeSvc.putEmployeeToApi(employee);
    };
}]).controller("deleteEmployeeCtrl", ['employeeSvc', function (employeeSvc) {
    employeeSvc.deleteEmployeeFromApi();
